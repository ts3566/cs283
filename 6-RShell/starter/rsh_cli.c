...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        io_size = send(cli_socket, cmd_buff, cmd_len + 1, 0);
        if (io_size < 0) {
            perror("send");
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
        }

        // Receive response from server until EOF character
        while (1) {
            ...
            if (is_eof) {
                io_size--; // Don't print the EOF character
            }
        ...
        // Check if command was "exit" or "stop-server"
